...
struct
  ...
  let normalize
    ...
    begin match x with
      | Top -> Top
      | Vals (vps,(t,_)) -> Vals (vps,(t,TermClassification.Introduction))
    end
...
struct
  ...
  let print_id id =
    ...
    | VariantConstruct x -> "Variant(" ^ CoreAndMore.Id.to_string x ^ ")"
      ...
      let rec term_cost_internal
        ...
        if List.mem ~equal:A.Term.equal !terms fullt then
          ...
          begin match Transition.id t with
            ...
            | Apply ->
              simple_subcost ts
            | VariantConstruct _ -> 2. +. simple_subcost ts
            | UnsafeVariantDestruct _ ->
              1. +. simple_subcost ts
            | TupleConstruct _ ->
              1. +. simple_subcost ts
