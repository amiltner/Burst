...
module TimbukBuilder : AutomatonBuilder =
  ...
  struct
    ...
    struct
      ...
      include Automaton.Make(Symb)(St)
    ...
    let compare_terms
        ((c1,r1s,_):Float.t * (Lang.Value.t * Lang.Value.t) list * TermState.t)
        ((c2,r2s,_):Float.t * (Lang.Value.t * Lang.Value.t) list * TermState.t)
      : comparison =
      let cc =
        Float.(
          if c1 <. c2 then
    ...
    let compare_terms
        ((c1,b1,_):Float.t * bool * TermState.t)
        ((c2,b2,_):Float.t * bool * TermState.t)
      : comparison =
      let cc =
        Float.(
          if c1 <. c2 then
