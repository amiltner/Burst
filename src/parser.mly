...
let mk_unctor_or_ctor_by_name
      ...
      (e:Expr.t)
    : Expr.t =
  if String.length c > 3 && String.equal (CoreAndMore.String.sub c ~pos:0 ~len:3) "Un_" then
    let c = CoreAndMore.String.sub c ~pos:3 ~len:(String.length c - 3) in
    Expr.mk_unctor (CoreAndMore.Id.create c) e
  else
    Expr.mk_ctor (CoreAndMore.Id.create c) e
%}
...
decl:
  | TYPE i=LID EQ t=typ
                      { Declaration.type_dec (CoreAndMore.Id.create i) t }
  | LET i=LID EQ e=exp SEMI SEMI
                     { Declaration.expr_dec (CoreAndMore.Id.create i) e }
...
typ_mu:
  | MU i=LID DOT t=typ { Type.mk_mu (CoreAndMore.Id.create i) t }
...
typ_single_variant:
  | PIPE i=UID OF t=typ_non_variant
    { (CoreAndMore.Id.create i,t) }
  | PIPE i=UID
    { (CoreAndMore.Id.create i,Type.mk_tuple []) }
...
typ_base:
  | d=LID { Type.mk_named (CoreAndMore.Id.create d) }
...
exp_base:
  | x=LID
        { Expr.mk_var (CoreAndMore.Id.create x) }
  ...
  | c=UID x=LID                                           (* Sugar: ctor with var argument.    *)
              { mk_unctor_or_ctor_by_name c (Expr.mk_var (CoreAndMore.Id.create x)) }
...
pattern:
  | c=UID
        { (Pattern.Ctor (CoreAndMore.Id.create c,Pattern.Wildcard)) }
  | c=UID p=pattern
              { (Pattern.Ctor (CoreAndMore.Id.create c,p)) }
  | WILDCARD
    { Pattern.Wildcard }
  | LPAREN RPAREN
    { Pattern.Tuple [] }
  ...
  | x=LID
        { Pattern.Var (CoreAndMore.Id.create x) }
